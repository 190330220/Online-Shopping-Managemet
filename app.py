...
def login():
    ...
    if request.method == 'POST':    # Create variables for easy access
        ...
        conn = mysql.connector.connect(host='localhost', user='root', password='shayobanazshaik', database='pfsd')
        ...
        if account:
            if(account[3]=="admin@easyshop.in"):
...
def Adminpage():
    ...
    cursor.execute('SELECT * FROM customer where email!="admin@easyshop.in"')
...
def orders():
    ...
    cursor.execute('select * from orders,customer where orders.cusid=customer.cusid')
    data = cursor.fetchall()
...
def signup():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        conn = mysql.connector.connect(host='localhost',user='root',password='shayobanazshaik',database='myshop')
        #cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor=conn.cursor()
        sql="insert into Customers(firstname,lastname,name,email,password)values(%s,%s,%s,%s)"
...
def add_product_to_cart():
    ...
    try:
        ...
        if _quantity and _code and request.method == 'POST':
            conn =  conn = mysql.connector.connect(host='localhost',user='root',password='shayobanazshaik',database='pfsd')
            cursor = conn.cursor()
           ...
           # itemArray = {
            #    row['code']: {'name': row['name'], 'code': row['code'], 'quantity': _quantity, 'price': row['price'],
             #                 'images': row['images'], 'total_price': _quantity * row['price']}}
            itemArray = {row[2]: {'name': row[1], 'code': row[2], 'quantity': _quantity, 'price': row[4],
                              'images': row[3], 'total_price': _quantity * row[4]}}
            all_total_price = 0
        ...
        if 'cart_item' in session:
            if request.method == 'POST':
                for key, value in session['cart_item'].items():
                    ...
                    conn = conn = mysql.connector.connect(host='localhost', user='root', password='shayobanazshaik',
                                                          database='pfsd')
                    cursor = conn.cursor()
